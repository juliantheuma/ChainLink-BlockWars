...
public class TwinStickMovement : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        if(Mathf.Abs(move.magnitude)> 0 && !isRunning){
            ...
            animator.SetBool("isRunning", false); //TO CHANGE, animation rig
            animator.SetInteger("animIndex", 0); //reset animation index to default to prevent loop
            isRunning = false;
        }
        }
        //fall down
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit){
        ...
        if(myPhotonPlayer.IsMine){
            IInGameInventoryItem item = hit.collider.GetComponent<IInGameInventoryItem>();
           if(item != null)
           {
